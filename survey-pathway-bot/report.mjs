#!/usr/bin/env node
// Turns a directory of run traces into a QA report: pathway flow graph,
// per-option coverage, and the list of things that went wrong.
//
//   node report.mjs [outDir]        # re-render ./out/REPORT.md from ./out/runs
//
// explore.mjs and run-paths.mjs call buildReport() directly.
...
const nodeId = (fp) => 'N' + Buffer.from(String(fp)).toString('hex').slice(0, 12);

export function buildReport(traces, summary = {}) {
  const L = [];
